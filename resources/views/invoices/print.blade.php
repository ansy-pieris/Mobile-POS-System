...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Developer Credit - Cannot be edited */
        .developer-credit {
            margin-top: 30px;
            padding-top: 15px;
...
<body>
    ...
    <div class="invoice-info">
        ...
        <div class="info-box payment">
            ...
            <div class="info-text">
                ...
                Salesperson: {{ $invoice->user->name ?? 'N/A' }}
            </div>
        </div>
    </div>
        ...
        <tbody>
            @foreach($invoice->items as $index => $item)
                @php
                    ...
                    $itemTotal = $item->total ?? ($itemSubtotal - $discountAmount);
                @endphp
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>
                        <strong>{{ $item->product->product_code ?? 'N/A' }}</strong>
                    </td>
                    <td>
                        {{ $item->description }}
                        @if($item->warranty_months > 0)
                            <span class="warranty-badge">{{ $item->warranty_months }}M Warranty</span>
                        @endif
                    </td>
                    ...
                    <td class="text-right">
                        ...
                        @if($discountAmount > 0)
                            <br><span class="discount-text">-Rs.{{ number_format($discountAmount / $item->quantity, 2) }}</span>
                        @endif
                    </td>
                    <td class="text-right">
    ...
    <!-- Terms & Conditions - Right after table -->
    ...
    <div class="developer-credit" style="margin-top: 40px; padding: 20px; background: #f8fafc; border-radius: 8px; text-align: center; font-size: 12px; color: #475569; border: 1px solid #e2e8f0;">
        <strong style="color: #1e40af; font-size: 13px;">This invoice has been created by Nexora Solutions</strong><br>
